...
|---|---:|---|
| Continuous microphone capture from launch | Medium | Open; record-state gating required |
...
passes fixed-size audio blocks to Caustic's recording callback. Wrapper buffers
are fixed and reused, and the wrapper does not save audio files, but this has
privacy, battery, and performance implications. Use the Vita hardware mute when
microphone input is not needed until on-demand capture is implemented.
