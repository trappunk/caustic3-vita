...
void so_flush_caches(so_module *mod) {
    ...
    for (int i = 0; i < mod->n_data; i++)
        kuKernelFlushCaches((void *)mod->data_base[i], mod->data_size[i]);
}

static uintptr_t so_read_word(const uintptr_t *ptr) {
    uintptr_t value;
    kuKernelCpuUnrestrictedMemcpy(&value, ptr, sizeof(value));
    return value;
}

static void so_write_word(so_module *mod, uintptr_t *ptr, uintptr_t value) {
    uintptr_t address = (uintptr_t)ptr;
    for (int i = 0; i < mod->n_data; i++) {
        if (address >= mod->data_base[i] &&
            address + sizeof(value) <= mod->data_base[i] + mod->data_size[i]) {
            *(volatile uintptr_t *)ptr = value;
            return;
        }
    }
    kuKernelCpuUnrestrictedMemcpy(ptr, &value, sizeof(value));
}
...
int _so_load(so_module *mod, SceUID so_blockid, void *so_data, uintptr_t load_addr) {
    ...
    /* text_base is the ELF load bias.  Android DSOs may put a read-only
     * PT_LOAD before their executable PT_LOAD, so the latter is not
     * necessarily at virtual address zero. */
    mod->text_base = load_addr;

    for (int i = 0; i < mod->ehdr->e_phnum; i++) {
        if (mod->phdr[i].p_type == PT_LOAD) {
            void *prog_data;
            const uintptr_t vaddr = mod->phdr[i].p_vaddr;
            const size_t align = mod->phdr[i].p_align ? mod->phdr[i].p_align : 0x1000;
            const uintptr_t seg_vaddr = vaddr & ~(align - 1);
            const size_t seg_offset = vaddr - seg_vaddr;
            const size_t prog_size = ALIGN_MEM(seg_offset + mod->phdr[i].p_memsz, align);
            const uintptr_t desired_addr = load_addr + seg_vaddr;
            ...
            if ((mod->phdr[i].p_flags & PF_X) == PF_X) {
                ...
                opt.size = sizeof(SceKernelAllocMemBlockKernelOpt);
                opt.attr = 0x1;
                opt.field_C = (SceUInt32)desired_addr;
                ...
                mod->exec_base = (uintptr_t)prog_data + seg_offset;
                ...
                mod->cave_size = (prog_size - (seg_offset + mod->phdr[i].p_memsz)) & ~0x3;
                mod->cave_base = mod->cave_head = (uintptr_t)prog_data + seg_offset + mod->phdr[i].p_memsz;
                ...
                mod->data_base[mod->n_data] = (uintptr_t)prog_data + seg_offset;
                mod->data_size[mod->n_data] = mod->phdr[i].p_memsz;
                mod->n_data++;
            }

            void *destination = (void *)(load_addr + vaddr);
            kuKernelCpuUnrestrictedMemcpy(destination, (void *)((uintptr_t)so_data + mod->phdr[i].p_offset), mod->phdr[i].p_filesz);
            static const unsigned char zero[4096] = {0};
            size_t remaining = mod->phdr[i].p_memsz - mod->phdr[i].p_filesz;
            uintptr_t zero_at = (uintptr_t)destination + mod->phdr[i].p_filesz;
            while (remaining) {
                size_t amount = remaining < sizeof(zero) ? remaining : sizeof(zero);
                kuKernelCpuUnrestrictedMemcpy((void *)zero_at, zero, amount);
                zero_at += amount;
                remaining -= amount;
            }
        }
    ...
    /* Segment contents were written through kubridge.  Synchronize them
     * before parsing mapped sections or reading implicit REL/RELR addends;
     * otherwise the user CPU can observe stale data from these fixed-address
     * blocks and relocate arbitrary values. */
    so_flush_caches(mod);
    ...
    for (int i = 0; i < mod->ehdr->e_shnum; i++) {
        ...
        if (strcmp(sh_name, ".dynamic") == 0) {
            ...
            mod->relrdyn = (Elf32_Addr *)sh_addr;
            mod->num_relrdyn = sh_size / sizeof(Elf32_Addr);
...
int so_relocate(so_module *mod) {
    ...
    for (int i = 0; i < mod->num_reldyn + mod->num_relplt; i++) {
        ...
        switch (type) {
            case R_ARM_NONE:
                /* Linkers may leave no-op relocation slots in .rel.dyn. */
                break;
            case R_ARM_ABS32:
                if (sym->st_shndx != SHN_UNDEF) {
                    val = so_read_word(ptr) + mod->text_base + sym->st_value;
                    so_write_word(mod, ptr, val);
                }
                break;
            case R_ARM_RELATIVE:
                val = so_read_word(ptr) + mod->text_base;
                so_write_word(mod, ptr, val);
                break;
            case R_ARM_GLOB_DAT:
    ...
    /* Android's newer linker packs relative relocations into SHT_RELR.  An
     * even entry selects a new relocation address; an odd entry is a bitmap
     * covering the following 31 words. */
    uintptr_t *where = NULL;
    for (int i = 0; i < mod->num_relrdyn; i++) {
        Elf32_Addr entry = mod->relrdyn[i];
        if ((entry & 1) == 0) {
            where = (uintptr_t *)(mod->text_base + entry);
            val = so_read_word(where) + mod->text_base;
            so_write_word(mod, where, val);
            where++;
        } else {
            if (where == NULL)
                return -1;
            for (unsigned bit = 1; bit < 32; bit++) {
                if (entry & ((Elf32_Addr)1 << bit)) {
                    uintptr_t *ptr = where + (bit - 1);
                    val = so_read_word(ptr) + mod->text_base;
                    so_write_word(mod, ptr, val);
                }
            }
...
{
    /* A bounded scan is cheap for mobile ELFs and robust when a loader or
     * emulator does not preserve the legacy SysV hash chain exactly. */
    for (int i = 0; i < mod->num_dynsym; i++) {
        if (mod->dynsym[i].st_shndx == SHN_UNDEF)
            continue;
        if (mod->dynsym[i].st_name >= mod->dynstr_size)
            continue;
        const char *name = mod->dynstr + mod->dynsym[i].st_name;
        if (!memchr(name, '\0', mod->dynstr_size - mod->dynsym[i].st_name))
            continue;
        if (mod->dynsym[i].st_info != SHN_UNDEF && strcmp(name, symbol) == 0)
            return i;
    }
