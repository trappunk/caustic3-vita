...
void glShaderSourceHook(GLuint shader, GLsizei count, const GLchar **string,
                        const GLint *_length) {
    debug_log("Caustic3 Vita: glShaderSource begin");
    if (!string) {
    ...
    /* Caustic ships plain GLES2 shaders. The old wrapper inherited a Cg-style
       samplerCUBE helper from a different game; injecting it made VitaGL's
       program linker crash even though the individual stages compiled. */
    const GLchar *source = str;
    GLint source_length = total_length;
    glShaderSource(shader, 1, &source, &source_length);
    free(str);
    debug_log("Caustic3 Vita: glShaderSource returned");
}

void glCompileShaderHook(GLuint shader) {
    debug_log("Caustic3 Vita: glCompileShader begin");
    if (!skip_next_compile) {
        glCompileShader(shader);
        debug_log("Caustic3 Vita: glCompileShader returned");
    }
    skip_next_compile = GL_FALSE;
    debug_log("Caustic3 Vita: glCompileShader hook complete");
}
